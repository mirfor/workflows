...
def test_A003_empty_do_container_body() -> None:
    ...
    wf = _wf([
        {"empty_try": TryTask.model_construct(try_=[], catch={"as": "e"}).model_dump(by_alias=True)},
    ])
    rep = validate(wf)
...
def test_A011_empty_fork_branches() -> None:
    ...
    wf = _wf([
        {"par": ForkTask.model_construct(fork={"branches": [], "compete": False}).model_dump(by_alias=True)},
    ])
    rep = validate(wf)
...
def test_C002_call_unknown_function() -> None:
    ...
    wf = _wf([
        {"send": CallTask(call="undefined_fn", timeout="default_timeout").model_dump(by_alias=True)},
    ], use=use)
    rep = validate(wf)
...
def test_C003_catch_unknown_error_type() -> None:
    ...
    use = Use(functions={
        "send": ToolFunction(name="send", type="weaver_tool", module="m", operation="o", errors=[]),
    })
    wf = _wf([
        {"wrap": TryTask(
...
def test_C003_accepts_tool_declared_error() -> None:
    ...
    use = Use(functions={
        "send": ToolFunction(
            name="send", type="weaver_tool", module="m", operation="o",
...
def test_C010_unknown_timeout_profile_ref() -> None:
    use = Use(functions={"f": ToolFunction(name="f", type="weaver_tool", module="m", operation="o")})
    wf = _wf([
...
def test_F101_workflow_run_timeout_must_be_string() -> None:
    ...
    wf = _wf([{"x": SetTask(set={}).model_dump(by_alias=True)}],
             metadata={"temporal": {"workflow_run_timeout": 60}})  # int zamiast str
    rep = validate(wf)
...
def test_apply_default_timeout_includes_schedule_to_close() -> None:
    wf = _wf([])
    apply_default_timeout(wf, cascade_after="PT3M",
                          cascade_heartbeat=None, cascade_schedule_to_close="PT10M")
...
def test_apply_default_timeout_no_metadata_when_no_extras() -> None:
    wf = _wf([])
    apply_default_timeout(wf, cascade_after="PT2M",
                          cascade_heartbeat=None, cascade_schedule_to_close=None)
